...
class DSTANet(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_frame=64, num_subset=3, dropout=0., config=None, num_person=2,
    ...
    def forward(self, x, drop_graph=False, return1d=False, KNN=False, return_feat=False):
        ...
        # NM, C, T, V
        feature = x.view(N, self.out_channels, -1, V, M)
        # global pooling
...
if __name__ == '__main__':
    ...
    ske = torch.rand([2, 3, 64, 25, 2])  # .cuda()
    out, f = net(ske)
    print(out.shape, f.shape)
