...
class ST_Transformer(nn.Module):
    ...
    def __init__(self, num_frame=64, num_joint=50, input_channel=3, dim_joint_emb=32, hidden_dim=256, num_class=60,
                depth=4, num_heads=8, qkv_bias=True, ff_expand=2.0, do_rate=0.1, attn_do_rate=0.1,
                drop_path_rate=0.1, add_positional_emb=1, positional_emb_type='learnalbe'):
    ...
    def forward(self, x):
        ...
        feature = x.view(b, t//4, v, self.hidden_dim)
        # print(feature.shape)
        x_pool = feature.permute(0, 3, 1, 2).contiguous()
        x_pool = F.adaptive_avg_pool2d(x_pool, 1).flatten(2).squeeze(-1)
        ## MLP
        out = self.fc(x_pool)
        # x = self.mlp(x)
        return out, feature
...
if __name__ == '__main__':
    x = torch.randn((16, 3, 64, 25, 2)).cuda()
    test = ST_Transformer()
    test = test.cuda()
    z, _ = test(x)
    print(z.shape)