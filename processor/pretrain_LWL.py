...
class SkeletonCLR_Processor(PT_Processor):
    ...
    def train(self, epoch):
        ...
        for [data1, data2, index], label in loader:
            ...
            # forward
            logits0, label0, logits1, label1 = self.model(data1, data2)
            if hasattr(self.model, 'module'):
                self.model.module.update_ptr(logits0.size(0))
            else:
                self.model.update_ptr(logits0.size(0))
            loss = self.multi_nce_loss(logits0, label0) + self.multi_nce_loss(logits1, label1)
